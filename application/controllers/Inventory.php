	...
	*/
	public function index()
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules('code', 'Code', 'alpha_numeric|trim');
			...
			$this->form_validation->set_rules('qty', 'Qty', 'numeric|trim');
				...
				if ($this->form_validation->run() === TRUE) {
					...
					if ($this->inventory_model->insert_data($data)) {
						...
						if (!empty($_FILES['photo']['name'])) {
							$config['file_name']     = trim($newKode.str_replace(" ", "_", $this->input->post('brand')).str_replace(" ", "_", $this->input->post('model')));
	...
	public function search($process="")
	{
		...
		// Logged in
			...
			$this->form_validation->set_rules(
				'keyword',
				'Keyword',
				'alpha_numeric_spaces|trim|required|min_length[2]'
			);
			...
			if (isset($_POST) && !empty($_POST)) {
				...
				if ($this->form_validation->run() === TRUE) {
					...
					if ($this->inventory_model->update_inventory_by_code($code, $data)) {
						...
						$link_thumbnail = "";
						if (!empty($_FILES['photo']['name'])) {
							$config['file_name']     = trim($newKode.str_replace(" ", "_", $this->input->post('brand')).str_replace(" ", "_", $this->input->post('model')).rand());
							$config['upload_path']   = './assets/uploads/images/inventory/';
							$config['allowed_types'] = 'gif|jpg|png';
							$config['max_size']      = 2048;
							$config['overwrite']     = TRUE;
	...
	public function delete($id)
	{
		...
		else
		{
			...
			if (isset($_POST) && !empty($_POST)) {
				...
				if ($this->form_validation->run() === TRUE) {
					$where = array('id' => $id);
					$this->inventory_model->delete_data($where,'inv_datas');
